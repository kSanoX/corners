...
let selected = null;
let currentPlayer = 1;
let hasJumpedYet = false; // Флаг для отслеживания, что был сделан прыжок
let canChangePiece = true; // Флаг, разрешающий смену шашки
...
function renderBoard() {
  ...
  endTurnBtn.disabled = !selected || !canChangePiece; // Кнопка активна только если есть выбранная шашка и можно сменить шашку
}
...
function onCellClick(e) {
    ...
    if (selected) {
      if (isValidJump(selected.r, selected.c, r, c)) {
        ...
        // Нельзя сделать неправильный ход — просто игнорируем
        return;
      }
...
function endTurn() {
    hasJumpedYet = false;
    canChangePiece = true; // Разрешаем смену шашки на следующий ход
    endTurnBtn.disabled = true;
    selected = null;

    if (checkWin(currentPlayer)) {
        statusEl.textContent = `Виграв гравець ${currentPlayer}!`;
...
function isValidJump(fromR, fromC, toR, toC) {
    ...
    return (
      midR >= 0 && midR < size &&
      midC >= 0 && midC < size &&
      board[midR][midC] !== null &&  // можно перепрыгнуть через любую фигуру
      board[toR][toC] === null
    );
  }
